  ...
  return merge.recursive(Field(), {
    ...
    computed: {
      ...
      filteredItems() {
        ...
        return this.allItems.filter(item => {
          return (
            !item[this.filterBy] || item[this.filterBy] == this.filterValue
          );
        });
      },
      flatItems() {
    ...
    methods: {
      ...
      initSelect2() {
        if (typeof $ == "undefined") {
          console.error(
            "vue-form-2: missing global dependency: vf-select with select2 depends on JQuery"
          );
          return;
        }
        ...
        if (typeof window.$().select2 === "undefined") {
          console.error(
            "vue-form-2: missing global dependency: vf-select with select2 depends on Select2"
          );
          return;
        }

        var options = this.inForm()
          ? clone(this.getForm().opts.select2Options)
          : {};
        var self = this;
          ...
          options = merge.recursive(options, {
            ajax: {
              ...
              data: function(params) {
                ...
                if (this.filterBy) {
                  var filterValue = self.getForm().getField(this.filterBy)
                    .value;
        ...
        if (this.containerClass) {
          this.el
            .data("select2")
            .$container.addClass("container-" + this.containerClass);
          this.el
            .data("select2")
            .$dropdown.addClass("dropdown-" + this.containerClass);
        }
      },
      removeDuplicateValues() {
        ...
        this.$nextTick(() => {
          $(this.$el)
            .find(".select2-selection__rendered li")
            .each(function() {
              title = $(this).prop("title");
              $(this)
                .siblings(`[title='${title}']`)
                .remove();
            });
        });
      },
