    ...
    computed: {
      ...
      togglerText: function() {
        return this.expanded
          ? this.togglerButton.minimize
          : this.togglerButton.expand;
      }
    },
    mounted: function() {
      ...
      if (typeof tinymce == "undefined") {
        console.error(
          "vue-form-2: missing global dependency. TinyMCE is required on a vf-textarea with a tinymce prop"
        );
        return;
      }
      ...
      var setup =
        that.options && that.options.hasOwnProperty("setup")
          ? that.options.setup
          : function() {};

      var parentSetup = that
        .getForm()
        .opts.tinymceOptions.hasOwnProperty("setup")
        ? that.getForm().opts.tinymceOptions.setup
        : function() {};

      var options = merge.recursive(
        clone(this.getForm().opts.tinymceOptions),
        this.options
      );
