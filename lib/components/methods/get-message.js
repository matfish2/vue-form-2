module.exports = function(rule) {
  ...
  let message = this.messages[rule] ? this.messages[rule] : messages[rule];
    ...
    params.forEach(
      function(param, index) {
        ...
        message = message.replace(
          "{" + index + "}",
          !isMomentObject(param) ? param : param.format(this.format)
        );
      }.bind(this)
    );
  ...
  message = message.replace(
    ":field",
    this.label ? stripLabel(this.label) : this.Name
  );
...
function extractMessage(rule, message, field) {
  ...
  if (
    ["date", "partialdate", "datepicker", "pikaday"].indexOf(field.fieldType) >
    -1
  )
    return message.date;
